...
if (seccion === 'ssbu') {
    cargarPersonajesSSBU();
}
}
// ============================================================
// MÓDULO DE SSBU (Super Smash Bros. Ultimate)
// ============================================================

const formSSBUPersonaje       = document.getElementById('form-ssbu-personaje');
const inputSSBUPersonajeId    = document.getElementById('ssbu-personaje-id');
const inputSSBUNombre         = document.getElementById('ssbu-nombre');
const inputSSBUSaga           = document.getElementById('ssbu-saga');
const inputSSBUPeso           = document.getElementById('ssbu-peso');
const ssbuFormTitulo          = document.getElementById('ssbu-form-titulo');
const btnGuardarSSBU          = document.getElementById('btn-guardar-ssbu');
const btnCancelarSSBU         = document.getElementById('btn-cancelar-ssbu');
const tbodySSBUPersonajes     = document.getElementById('tbody-ssbu-personajes');
const tablaSSBUPersonajes     = document.getElementById('tabla-ssbu-personajes');
...
async function cargarPersonajesSSBU() {
    ...
    try {
        ...
        const resp = await fetchAPI('/api/ssbu/personajes');

        tbodySSBUPersonajes.innerHTML = '';

        if (resp.data.length === 0) {

            cargaSSBUPersonajes.textContent = 'No hay personajes registrados.';
            cargaSSBUPersonajes.style.display = 'block';
            tablaSSBUPersonajes.style.display = 'none';
            return;

        }

        cargaSSBUPersonajes.style.display = 'none';
        tablaSSBUPersonajes.style.display = 'table';
        ...
        resp.data.forEach(p => {
            ...
            fila.innerHTML = `
                <td>${p.id}</td>
                <td>${escapeHtml(p.nombre)}</td>
                <td>${escapeHtml(p.saga)}</td>
                <td>${p.peso}</td>
                <td>
                    <button class="btn-editar"
                        onclick="editarPersonajeSSBU(${p.id})">
                        Editar
                    </button>
                    <button class="btn-eliminar"
                        onclick="eliminarPersonajeSSBU(${p.id})">
                        Eliminar
                    </button>
                </td>
            `;

            tbodySSBUPersonajes.appendChild(fila);

        });

        contadorSSBUPersonajes.textContent = `${resp.count}`;
...
async function editarPersonajeSSBU(id) {

    try {

        const resp = await fetchAPI(`/api/ssbu/personajes/${id}`);

        inputSSBUPersonajeId.value = resp.data.id;
        inputSSBUNombre.value      = resp.data.nombre;
        ...
        formSSBUPersonaje.scrollIntoView({ behavior: 'smooth' });
...
async function eliminarPersonajeSSBU(id) {

    if (!confirm('¿Eliminar personaje?')) return;

    try {

        await fetchAPI(`/api/ssbu/personajes/${id}`, { method: 'DELETE' });

        mostrarNotificacion('Personaje eliminado', 'exito');
        cargarPersonajesSSBU();
...
function limpiarFormSSBU() {

    formSSBUPersonaje.reset();
    inputSSBUPersonajeId.value    = '';
    ssbuFormTitulo.textContent    = 'Agregar Personaje';
...
formSSBUPersonaje.addEventListener('submit', async (e) => {
    ...
    const datos = {
        ...
        saga   : inputSSBUSaga.value.trim(),
        peso   : parseFloat(inputSSBUPeso.value)
    };

    try {

        if (inputSSBUPersonajeId.value) {

            await fetchAPI(
                `/api/ssbu/personajes/${inputSSBUPersonajeId.value}`,
                {
                    method  : 'PUT',
                    headers : { 'Content-Type': 'application/json' },
                    body    : JSON.stringify(datos)
            ...
            mostrarNotificacion('Personaje actualizado', 'exito');
            ...
            mostrarNotificacion('Personaje creado', 'exito');
